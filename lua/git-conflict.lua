...
local function shade_color(rgb_24bit, percent)
  ...
  local function alter(v) return math.min(math.floor(v * (100 + percent) / 100), 255) end
...
local function parse_buffer(bufnr)
  ...
  local prev_had = visited_buffers[name]
    and visited_buffers[name].positions
    and #visited_buffers[name].positions > 0
  local has_conflict, positions = detect_conflicts(lines)

  if not visited_buffers[name] then visited_buffers[name] = {} end
...
local function process(bufnr)
  ...
  if entry and entry.tick == vim.b[bufnr].changedtick then return end
  parse_buffer(bufnr)
end
...
local function find_position(bufnr, comparator, opts)
  ...
  if opts and opts.reverse then
    for i = #match.positions, 1, -1 do
      if comparator(line, match.positions[i]) then return match.positions[i] end
    end
...
local function choose(side)
  ...
  if position then resolve_position(bufnr, position, side) end
end
...
local function setup_buffer_mappings(bufnr)
  local function opts(desc) return { silent = true, buffer = bufnr, desc = 'Git Conflict: ' .. desc } end
  vim.keymap.set({ 'n', 'v' }, DEFAULT_MAPPINGS.ours, function() choose('ours') end, opts('Choose Ours'))
  vim.keymap.set({ 'n', 'v' }, DEFAULT_MAPPINGS.theirs, function() choose('theirs') end, opts('Choose Theirs'))
  vim.keymap.set({ 'n', 'v' }, DEFAULT_MAPPINGS.both, function() choose('both') end, opts('Choose Both'))
  vim.keymap.set({ 'n', 'v' }, DEFAULT_MAPPINGS.none, function() choose('none') end, opts('Choose None'))
  vim.keymap.set('n', DEFAULT_MAPPINGS.next, function() find_next('ours') end, opts('Next Conflict'))
  vim.keymap.set('n', DEFAULT_MAPPINGS.prev, function() find_prev('ours') end, opts('Previous Conflict'))
...
local function set_commands()
  ...
  cmd('GitConflictRefresh', function()
    for _, buf in ipairs(api.nvim_list_bufs()) do
      if api.nvim_buf_is_loaded(buf) and is_valid_buf(buf) then try_register(buf) end
    end
  end, {})
  cmd('GitConflictChooseOurs', function() choose('ours') end, {})
  cmd('GitConflictChooseTheirs', function() choose('theirs') end, {})
  cmd('GitConflictChooseBoth', function() choose('both') end, {})
  cmd('GitConflictChooseNone', function() choose('none') end, {})
  cmd('GitConflictNextConflict', function() find_next('ours') end, {})
  cmd('GitConflictPrevConflict', function() find_prev('ours') end, {})
end
...
api.nvim_create_autocmd('BufReadPost', {
  group = aug,
  callback = function(args) try_register(args.buf) end,
})
...
api.nvim_set_decoration_provider(NAMESPACE, {
  on_buf = function(_, bufnr) return is_valid_buf(bufnr) end,
...
try_register(api.nvim_get_current_buf())
